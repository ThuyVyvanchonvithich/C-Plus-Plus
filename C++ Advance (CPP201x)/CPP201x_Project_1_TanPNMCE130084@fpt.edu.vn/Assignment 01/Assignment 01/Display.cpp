...
#pragma region Input light level
	...
	do {
		...
		cin >> light_lv;
		//check input data
		if (!regex_match(light_lv, numberRegex)) {	//if is not a digit
...
#pragma region Input screen light level
	do {
		...
		cin >> screen_light_lv;
		//check input data
		if (!regex_match(screen_light_lv, numberRegex)) {	//if is not a digit
...
#pragma region Input Taplo light level
	...
	do {
		...
		cin >> taplo_light_lv;
		//check input data
		if (!regex_match(taplo_light_lv, numberRegex)) {	//if is not a digit
...
/*
* The function to insert empty Display
*/
Display Display::insertEmptyDisplay(Setting setting) {
	return Display(0, 0, 0, setting.getCarName(), setting.getPersonalKey(), setting.getEmail(), setting.getODO(), setting.getServiceRemind());
}
